  ...
  let ORPLStockAdjustments = {
    ...
    init: function () {
      ...
      if (typeof orplStockAdjustments !== "undefined") {
        ...
        this.config.getProductsNonce =
          orplStockAdjustments.getProductsNonce || "";
    ...
    bindEvents: function () {
      ...
      $("#adjustment-product").on("change", function () {
        ...
        var netCost = selectedOption.data("net-cost") || 0;

        $("#buy-price").text(parseFloat(netCost).toFixed(2));
      ...
      $("#adjustment-type, #adjustment-quantity").on(
        "change input",
        function () {
          self.calculateNewStock();
        },
      ...
      $(document).on("click", ".prev-page", function (e) {
        ...
        if (self.config.currentPage > 1)
          self.loadAdjustments(self.config.currentPage - 1);
      });
      ...
      $(document).on("click", ".last-page", function (e) {
        e.preventDefault();
        if (self.config.currentPage < self.config.totalPages)
          self.loadAdjustments(self.config.totalPages);
      });
        ...
        success: function (response) {
          if (response.success) {
            ...
            select
              ...
              .append(
                '<option value="">' +
                  self.config.strings.selectStock +
                  "</option>",
              );
            ...
            $.each(response.data, function (_, stock) {
              ...
              var option = $("<option>")
                .val(stock.stock_id)
                .text(displayText)
                .data("net-cost", stock.net_cost); // ← store buying price
    ...
    loadAdjustments: function (page) {
      ...
      tbody.html(
        '<tr><td colspan="6" class="loading-stocks"><span class="spinner is-active"></span> ' +
          (self.config.strings.loadingAdjustments || "Loading adjustments...") +
          "</td></tr>",
      );
      ...
      $.ajax({
        ...
        success: function (response) {
          ...
          if (response.success) {
            if (!response.data.adjustments.length) {
              tbody.append(
                '<tr><td colspan="6" class="no-stocks">' +
                  (self.config.strings.noAdjustments ||
            ...
            $.each(response.data.adjustments, function (_, adjustment) {
              ...
              let typeText =
                adjustment.adjustment_type === "increase"
                  ? self.config.strings.increase
                  : self.config.strings.decrease;
        ...
        error: function () {
          tbody.html(
            '<tr><td colspan="6" class="error-message">' +
              (self.config.strings.loadError || "Failed to load adjustments.") +
              "</td></tr>",
          );
        },
      });
    ...
    updatePagination: function (pagination) {
      ...
      $("#displaying-num").text(
        pagination.total_items + " " + self.config.strings.items,
      );
    ...
    handleDeleteAdjustment: function (button) {
      ...
      showLimeConfirm(
        self.config.strings.confirmDelete ||
          "Are you sure you want to delete this adjustment?",
        function onYes() {
          // Disable button and show deleting text
          $button
            .prop("disabled", true)
            .text(self.config.strings.deleting || "Deleting...");
          ...
          $.post(self.config.ajaxUrl, {
            ...
            .done(function (response) {
              if (response.success) {
                self.loadAdjustments(self.config.currentPage);
                showLimeModal(
                  response.data || "Adjustment deleted successfully.",
                  "Success",
                );
              } else {
            ...
            .fail(function () {
              $button.prop("disabled", false).text(originalText);
              showLimeModal(
                self.config.strings.deleteFailed ||
